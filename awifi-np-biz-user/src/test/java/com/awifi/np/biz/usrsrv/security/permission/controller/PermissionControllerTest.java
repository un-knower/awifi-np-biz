package com.awifi.np.biz.usrsrv.security.permission.controller;
...
@PrepareForTest({MessageUtil.class,SysConfigUtil.class,RedisUtil.class})
public class PermissionControllerTest {
    ...
    public void testCheckViewOK(){
        ...
        PowerMockito.when(RedisUtil.set(anyObject(),anyObject(),anyObject())).thenReturn("");
    ...
    public void pushForAm() throws Exception{
        PowerMockito.when(SysConfigUtil.getParamValue("servicecode_user")).thenReturn("servicecode");
        PowerMockito.when(SysConfigUtil.getParamValue("servicekey_user")).thenReturn("servicekey");
        Mockito.doNothing().when(permissionService).pushInterfaces(anyString(), anyString());
       
        Map<String,Object> resultMap = permissionController.pushForAm();
        String code = (String)resultMap.get("code");
        Assert.assertEquals(code, "0");
        PowerMockito.verifyStatic();
        SysConfigUtil.getParamValue("servicecode_user");
        SysConfigUtil.getParamValue("servicekey_user");
        permissionService.pushInterfaces(anyString(), anyString());
    }
}
