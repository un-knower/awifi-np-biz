...
var ComponentClass = function () {
    ...
    function setPageDoing(pageId) {
        pageDoing = pageId;
        _console.log('设备当前页面pageDoing=>', pageDoing);
    }
    ...
    function setEntityDoing(comp_id) {
        entityDoing = comp_id;
        _console.log('设置当前组件实体setEntityDoing=>', entityDoing);
    }
    ...
    function getEntityDoing() {
        _console.log('获取当前组件实体getEntityDoing=>', entityDoing);
        return entityDoing;
    }
    ...
    function pageAdd(page_type, page_id, pageOperation, pageId) {
        ...
        _console.log('添加页面结果add_page=>pagemap: ', pageMap);
    }
    ...
    function pageDel(page_type, page_id) {
        ...
        _console.log('删除页面结果pageDel=>pagemap: ', pageMap);
    }
    ...
    function addEntityData(data) {
        ...
        _console.log('添加组件实体结果addEntityData=>', entitylist);
    }
    ...
    function apiSave() {
        ...
        if (templateId && templateId > 0) {
            var url = '/template/addAll';
            var resp_data = {
                templateId: templateId,
                templateInfo: JSON.stringify(templateInfo.data)
            };
        } else if (siteId && siteId > 0) {
            var url = '/site/addAllSite';
            var resp_data = {
                siteId: siteId,
                siteInfo: JSON.stringify(templateInfo.data)
            };
        } else if (siteDefaultId && siteDefaultId > 0) {
            var url = '/siteDefault/addAllDefaultSite';
            var resp_data = {
    ...
    function pageEntityInit(data) {
        ...
        for (var i=0,len=componets.length; i<len; i++) {
            ...
            try {
                ...
                _console.log('[fault]pageEntityInit.addHeader=>', e);
                entityInitFault();
                return;
            }
            if (page_type == '1' || page_type == '3') {
    ...
    function apiGetComponentData() {
        var _type = 'get';
        if (templateId && templateId > 0) {
            var url = '/template/pageList';
            var resp_data = {
                templatePageId: templateId
            };
        } else if (siteId && siteId > 0) {
            var url = '/site/querySitePages';
            var resp_data = {
                siteId: siteId
            };
            _type = 'post';
        ...
        $.ajax({
            ...
            success: function (resp) {
                _console.log('获取不同类型组件json apiGetComponentData=>', resp);
                var data = resp.data;
     ...
     * @param type
     */
    function apiPageComponentListTodo(type) {
        type = type || 1;
        $.ajax({
            url: '/component/componentList',
            type: 'get',
            data: {
                "type": type,
                "projectId": gobal_projectId
            },
            dataType: 'json',
            async: false,
            success: function (resp) {
     ...
     * @param pageType
     */
    function apiPageComponentList(pageType, pageId) {
        pageType = pageType || 1;
        $.ajax({
            url: '/component/componentList',
            type: 'get',
    ...
    function createReact(el, comp_id, cpt_param) {
        ...
        var entity_htm = '<div class="entity-cover"></div>' +
    ...
    function createReactEntity(data) {
        ...
        if (!objectIsEmpty(compJson)) {
            ...
            var htm = '<li data-unique="' + compJson.canUnique + '" data-classify="' + compJson.classify + '" data-id="' + componentId + '" data-code="' + compJson.code + '" data-set-code="' + compJson.setCode + '" ' +
                ...
                '<div id="' + comp_id + '"></div>' +
                '<script type="text/javascript">var ' + comp_id + ' = ' + compJson.code + '(' + JSON.stringify(data.json) + ', "' + comp_id + '").render();</script></li>';
    ...
    var get_args = getArgs(),
        templateId = get_args['templateId'], // 模板id
    ...
    function init() {
        ...
        if (isSite()) {
            ...
            $('#btn-preview').prop({'href': '/static/html/preview.html?templateId='+templateId, 'target': '_blank'});
        }
        hideCompTip();

        // 创建模板或站点首次进入认证页
        if (!templateId && !siteId && !siteDefaultId && location.pathname.indexOf('test.html') == -1) {
            jDialog.alert('提示', '非法操作');
