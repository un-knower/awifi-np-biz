...
var ComponentClass = function () {
     ...
     */
    function apiSave() {
        var primary_industry = $("#primary_industry").val();
        var secondary_industry = $("#secondary_industry").val();
        ...
        var resp_data = {
            "priIndustryCode":primary_industry,
            "secIndustryCode":secondary_industry,
            "siteName":siteName,
            "pages": templateInfo.data
        };
        //编辑保存接口
        if (siteId && siteId < 0) {
            var url = '/portalsrv/site/'+siteId+'/industry?access_token='+accessToken;
            $.ajax({
                url: url,
                type: 'put',
            ...
            var url = '/portalsrv/site/industry?access_token='+accessToken;
            $.ajax({
                url: url,
                type: 'post',
                data: JSON.stringify(resp_data),
    ...
    //显示一级行业
    getPrimaryIndustry(accessToken);
...
//一级行业
function getPrimaryIndustry(accessToken){
    var url ="/pubsrv/industrys?access_token="+accessToken;
    $.ajax({
        url: url,
        type: 'get',
        data: {},
        ...
        success: function (resp) {
            if (resp.code == '0') {
                setOptionLists(resp.data, $('#primary_industry') ,1);
            } else {
                jDialog.alert('提示', resp.message);
            }
        }
...
function getSecondIndustry(){
    console.log("344444444");
    var get_args = getArgs(),
        accessToken = get_args['access_token']; //获取access_token
    var primaryIndustry = $("#primary_industry").val();
    var url ="/pubsrv/industrys?access_token="+accessToken;
    $.ajax({
        url: url,
        type: 'get',
        data: {"parentcode": primaryIndustry},
        dataType: 'json',
        success: function (resp) {
            if (resp.code == '0') {
                setOptionLists(resp.data,$('#secondary_industry'), 0);
            } else {
                jDialog.alert('提示', resp.message);
            }
        }
...
function setOptionLists(data,$obj , flag){
    if(flag){
        $obj.html(" <option value=''>一级行业</option>");
    }else{
    ...
    for(var i=0; i<data.length;i++){
        var item = data[i];
        htm = htm + '<option value='+item.industryCode+'>'+item.industryName+'</option>';
    }
    $obj.append(htm);
}
